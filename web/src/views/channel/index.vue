...
export default {
  ...
  methods: {
    ...
    sendDevicePush: function(itemData) {
      console.log('sendDevicePush调用，itemData:', itemData)
      if (!this.$refs.devicePlayer) {
        console.error('devicePlayer引用不存在')
        this.$message.error('播放器组件未初始化')
        return
      }
      itemData.playLoading = true
      this.$store.dispatch('commonChanel/playChannel', itemData.gbId)
        .then((data) => {
          console.log('播放请求成功，返回数据:', data)
          if (!data) {
            throw new Error('播放请求返回数据为空')
          }
          itemData.streamId = data.stream
          this.$refs.devicePlayer.openDialog('media', itemData.gbId, {
            streamInfo: data,
            hasAudio: itemData.hasAudio !== undefined ? itemData.hasAudio : false
          })
          setTimeout(() => {
            this.initData()
          }, 1000)
        ...
        .catch((error) => {
          ...
          const errorMsg = error && error.message ? error.message : (typeof error === 'string' ? error : '播放请求失败')
          this.$message({
            showClose: true,
            message: errorMsg,
            type: 'error'
