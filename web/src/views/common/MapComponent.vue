...
export default {
    ...
    theme() {
      if (tileLayer) {
        ...
        tileLayer = new Tile({
          ...
          className: this.theme === 'dark' ? 'dark-map-layer' : 'ol-layer'
        })
        olMap.getLayers().insertAt(0, tileLayer)
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    initMap(){
      // 默认中心点 (经度, 纬度) - 广州市烟草专卖局
      let center = fromLonLat([113.3175, 23.1200])
      window.coordinateSystem = this.mapTileList[this.mapTileIndex].coordinateSystem
      // 如果有全局配置 window.mapParam.center，则使用全局配置
