...
export default {
  ...
  methods: {
    ...
    play: function(streamInfo, hasAudio) {
      console.log('play方法调用，streamInfo:', streamInfo, 'hasAudio:', hasAudio)
      if (!streamInfo) {
        console.error('streamInfo为空，无法播放')
        this.$message.error('流信息为空，无法播放')
        return
      }
      this.streamInfo = streamInfo
      this.hasAudio = hasAudio !== undefined ? hasAudio : false
      this.isLoging = false
      const videoUrl = this.getUrlByStreamInfo()
      if (!videoUrl) {
        console.error('无法获取播放URL，streamInfo:', streamInfo)
        this.$message.error('无法获取播放地址，请检查流信息')
        return
      }
      ...
      console.log('准备播放，videoUrl:', videoUrl, 'streamId:', this.streamId)
      this.playFromStreamInfo(false, streamInfo)
    },
    getUrlByStreamInfo() {
      console.log(this.streamInfo)
      if (!this.streamInfo) {
        console.error('streamInfo为空')
        return ''
      }
      ...
      if (location.protocol === 'https:') {
        url = streamInfo[this.player[this.activePlayer][1]]
      } else {
        url = streamInfo[this.player[this.activePlayer][0]]
      }
      // 如果获取的URL为空，尝试使用其他格式
      if (!url && streamInfo.ws_flv) {
        url = streamInfo.ws_flv
      }
      if (!url && streamInfo.flv) {
        url = streamInfo.flv
      }
      if (!url && streamInfo.rtc) {
        url = streamInfo.rtc
      }
      console.log('获取到的播放URL:', url)
      this.videoUrl = url
      return url
    },

    playFromStreamInfo: function(realHasAudio, streamInfo) {
      console.log('playFromStreamInfo调用，streamInfo:', streamInfo)
      const videoUrl = this.getUrlByStreamInfo()
      if (!videoUrl) {
        console.error('无法获取播放URL，streamInfo:', this.streamInfo)
        this.$message.error('无法获取播放地址，请检查流信息')
        return
      }
      console.log('设置showVideoDialog为true，videoUrl:', videoUrl, 'activePlayer:', this.activePlayer)
      this.showVideoDialog = true
      this.hasaudio = realHasAudio && this.hasaudio
      
      // 确保对话框显示
      this.$nextTick(() => {
        console.log('$nextTick回调，showVideoDialog:', this.showVideoDialog)
        console.log('播放器引用检查:', this.$refs[this.activePlayer])
        if (this.$refs[this.activePlayer]) {
          console.log('调用播放器play方法，URL:', videoUrl)
          this.$refs[this.activePlayer].play(videoUrl)
        } else {
          console.error('播放器组件引用不存在:', this.activePlayer, '所有refs:', Object.keys(this.$refs))
          // 尝试延迟加载
          setTimeout(() => {
            if (this.$refs[this.activePlayer]) {
              this.$refs[this.activePlayer].play(videoUrl)
            } else {
              console.error('播放器组件仍未加载，activePlayer:', this.activePlayer)
              this.$message.error('播放器组件加载失败，请刷新页面重试')
            }
          }, 500)
        }
      })
