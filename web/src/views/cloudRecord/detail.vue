            ...
            <ul v-if="detailFiles.length >0" v-infinite-scroll="infiniteScroll" class="infinite-list record-list">
              <li v-for="(item,index) in detailFiles" :key="index" class="infinite-list-item record-list-item">
                ...
                <a
                  ...
                  style="color: #006838;font-weight: 600;margin-left: 10px;"
                  target="_blank"
...
export default {
  ...
  methods: {
    ...
    playRecord() {
      ...
      let deviceText = this.stream // Default to stream ID
      
      queryOne(this.stream).then(data => {
        if (data) {
          deviceText = (data.parentName || '市烟草局') + ' ' + (data.name || data.gbName || this.stream)
        } else {
          deviceText = '市烟草局 ' + this.stream
        }
      }).catch(e => {
        ...
        this.$store.dispatch('cloudRecord/loadRecord', {
          ...
          .then(data => {
            this.playerTime = 0
            this.$refs.cloudRecordPlayer.setStreamInfo(data, this.detailFiles[this.chooseFileIndex].timeLen, this.detailFiles[this.chooseFileIndex].startTime, '中国烟草', deviceText)
          })
          .catch((error) => {
            console.log(error)
          })
