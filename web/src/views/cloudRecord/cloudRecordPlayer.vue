  ...
  <div id="cloudRecordPlayer" style="height: 100%">
    <div class="cloud-record-playBox" :style="playBoxStyle">
      ...
      <video
        ...
        @timeupdate="onDomTimeUpdate"
        @play="playingChange(true)"
        @pause="playingChange(false)"
  ...
  methods: {
    onDomTimeUpdate(e) {
      if (this.streamInfo && this.streamInfo.duration) {
        // e.target.currentTime is in seconds, duration is usually in ms from backend
        // But if streamInfo.duration is not set correctly for MP4 direct play, we might need to rely on video.duration
    ...
    changePlaySpeed(speed) {
      ...
      if (this.playerType === 'DOM') {
          if (this.$refs.recordVideoPlayer) {
              this.$refs.recordVideoPlayer.playbackRate = speed;
          }
          return;
      }
    ...
    stopPLay() {
      ...
      if (this.playerType === 'DOM') {
          if (this.$refs.recordVideoPlayer) {
              ...
              this.$refs.recordVideoPlayer.currentTime = 0;
          }
      } else if (this.$refs.recordVideoPlayer) {
        this.$refs.recordVideoPlayer.destroy()
      }
    ...
    setStreamInfo(streamInfo, timeLen, startTime, watermarkText, deviceId) {
      ...
      // Auto detect MP4
      if (this.videoUrl && (this.videoUrl.indexOf('.mp4') !== -1 || this.videoUrl.indexOf('.MP4') !== -1)) {
          this.playerType = 'DOM';
      } else {
    ...
    seekRecord(playSeekValue, callback) {
      if (this.playerType === 'DOM') {
          if (this.$refs.recordVideoPlayer) {
              this.$refs.recordVideoPlayer.currentTime = playSeekValue / 1000;
