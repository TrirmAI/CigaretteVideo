...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        // OpenAPI 健康检查接口不需要认证
        defaultExcludes.add("/openapi/v1/health");
        defaultExcludes.add("/openapi/v1/system/info");
        
        // 执法操作日志接口不需要认证 (临时)
        defaultExcludes.add("/api/ext/law-log/**");
        
        // 模拟日志生成接口不需要认证
        defaultExcludes.add("/api/log/mock/**");
        
        // 临时开放录像上传接口用于测试
        defaultExcludes.add("/api/recorder/upload");

        // 运维大盘与业务分析接口 (临时开放，用于开发测试)
        defaultExcludes.add("/api/ext/ops/**");
        defaultExcludes.add("/api/ext/business/**");
