...
public class ChannelProvider {
    ...
    public String queryListInCircleForH2(Map<String, Object> params ){
        StringBuilder sqlBuild = new StringBuilder();
        sqlBuild.append(BASE_SQL_FOR_CAMERA_DEVICE);
        sqlBuild.append(" where wdc.channel_type = 0 AND wdc.data_type != 2 AND (wdc.gb_ptz_type is null or  ( wdc.gb_ptz_type != 98 AND wdc.gb_ptz_type != 99)) " +
                " AND coalesce(wdc.gb_parent_id, wdc.parent_id) in (");
        ...
        // H2数据库使用Haversine公式计算距离（单位：米）
        // 6371000是地球半径（米）
        Double centerLongitude = (Double)params.get("centerLongitude");
        Double centerLatitude = (Double)params.get("centerLatitude");
        Double radius = (Double)params.get("radius");
        
        sqlBuild.append(" AND (6371000 * 2 * ASIN(SQRT(POWER(SIN((RADIANS(").append(centerLatitude)
                .append(") - RADIANS(coalesce(wdc.gb_latitude, wdc.latitude))) / 2), 2) + ")
                .append("COS(RADIANS(").append(centerLatitude).append(")) * COS(RADIANS(coalesce(wdc.gb_latitude, wdc.latitude))) * ")
                .append("POWER(SIN((RADIANS(").append(centerLongitude).append(") - RADIANS(coalesce(wdc.gb_longitude, wdc.longitude))) / 2), 2)))) < ").append(radius);
    ...
    public String queryListInPolygonForH2(Map<String, Object> params ){
        StringBuilder sqlBuild = new StringBuilder();
        sqlBuild.append(BASE_SQL_FOR_CAMERA_DEVICE);
        sqlBuild.append(" where wdc.channel_type = 0 AND wdc.data_type != 2 AND (wdc.gb_ptz_type is null or  ( wdc.gb_ptz_type != 98 AND wdc.gb_ptz_type != 99)) " +
                " AND coalesce(wdc.gb_parent_id, wdc.parent_id) in (");
        ...
        // H2数据库使用简单的边界框检查（bounding box）
        // 对于精确的多边形判断，H2不支持，这里使用边界框近似
        List<Point> pointList = (List<Point>)params.get("pointList");
        if (pointList != null && !pointList.isEmpty()) {
            double minLng = pointList.stream().mapToDouble(Point::getLng).min().orElse(0);
            double maxLng = pointList.stream().mapToDouble(Point::getLng).max().orElse(0);
            double minLat = pointList.stream().mapToDouble(Point::getLat).min().orElse(0);
            double maxLat = pointList.stream().mapToDouble(Point::getLat).max().orElse(0);
            
            sqlBuild.append(" AND coalesce(wdc.gb_longitude, wdc.longitude) >= ").append(minLng)
                    .append(" AND coalesce(wdc.gb_longitude, wdc.longitude) <= ").append(maxLng)
                    .append(" AND coalesce(wdc.gb_latitude, wdc.latitude) >= ").append(minLat)
                    .append(" AND coalesce(wdc.gb_latitude, wdc.latitude) <= ").append(maxLat);
        }
