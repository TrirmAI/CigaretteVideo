...
public interface StreamPushMapper {
    ...
    @Select("SELECT st.*, st.id as data_device_id, wdc.*, COALESCE(wdc.id, 0) as gb_id FROM wvp_stream_push st LEFT join wvp_device_channel wdc on  wdc.data_type = 2 and st.id = wdc.data_device_id WHERE st.app=#{app} AND st.stream=#{stream}")
    ...
    @Select("SELECT st.*, st.id as data_device_id, wdc.*, COALESCE(wdc.id, 0) as gb_id FROM wvp_stream_push st LEFT join wvp_device_channel wdc on wdc.data_type = 2 and st.id = wdc.data_device_id WHERE st.media_server_id=#{mediaServerId}")
    List<StreamPush> selectAllByMediaServerId(String mediaServerId);

    @Select("SELECT st.*, st.id as data_device_id, wdc.*, COALESCE(wdc.id, 0) as gb_id FROM wvp_stream_push st LEFT join wvp_device_channel wdc on wdc.data_type = 2 and st.id = wdc.data_device_id WHERE st.media_server_id=#{mediaServerId} and wdc.gb_device_id is null")
    ...
    @Select("<script> "+
            "SELECT st.*, st.id as data_device_id, wdc.*, COALESCE(wdc.id, 0) as gb_id FROM wvp_stream_push st LEFT join wvp_device_channel wdc on wdc.data_type = 2 and st.id = wdc.data_device_id " +
    ...
    @MapKey("gbDeviceId")
    @Select("SELECT wdc.gb_device_id, wsp.id as data_device_id, wsp.*, wsp.* , COALESCE(wdc.id, 0) as gb_id " +
            " from wvp_stream_push wsp " +
            " LEFT join wvp_device_channel wdc on wdc.data_type = 2 and wsp.id = wdc.data_device_id")
    Map<String, StreamPush> getAllGBId();

    @Select("SELECT st.*, st.id as data_device_id, wdc.*, COALESCE(wdc.id, 0) as gb_id FROM wvp_stream_push st LEFT join wvp_device_channel wdc on wdc.data_type = 2 and st.id = wdc.data_device_id WHERE st.id=#{id}")
